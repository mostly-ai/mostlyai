...
class Generator:
    ...
    def reports(self, file_path: str | Path | None = None, display: bool = False) -> Path | None:
        ...
        Args:
            file_path: The file path to save the reports (ignored if display=True).
        ...
        for table in self.tables:
            if table.tabular_model_metrics:
                report = self.client._report(
                    generator_id=self.id,
                    source_table_id=table.id,
                    model_type="TABULAR",
                    short_lived_file_token=(self.metadata.short_lived_file_token if self.metadata else None),
                )
                html_for_display += f'<p><iframe srcdoc="{html.escape(report)}" width="100%" height="500"></iframe></p>'
                reports[f"{table.name}-tabular.html"] = report
            if table.language_model_metrics:
                ...
                html_for_display += f'<p><iframe srcdoc="{html.escape(report)}" width="100%" height="500"></iframe></p>'
                reports[f"{table.name}-language.html"] = report
        if display:
            if not rich.console._is_jupyter():
                import warnings

                warnings.filterwarnings("default", category=UserWarning)
                warnings.warn("`reports(..., display=True)` is only effective when in Jupyter notebook.")
            else:
        ...
        for report_type in [SyntheticDatasetReportType.model, SyntheticDatasetReportType.data]:
            ...
            for table in self.tables:
                if table.tabular_model_metrics:
                    ...
                    html_for_display += (
                        f'<p><iframe srcdoc="{html.escape(report)}" width="100%" height="500"></iframe></p>'
                    )
                    reports[f"{table.name}-tabular{report_infix}.html"] = report
                if table.language_model_metrics:
                    report = self.client._report(
                        synthetic_dataset_id=self.id,
                        synthetic_table_id=table.id,
                    ...
                    reports[f"{table.name}-language{report_infix}.html"] = report
        if display:
            if not rich.console._is_jupyter():
                import warnings
