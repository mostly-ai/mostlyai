...
#   timestamp: 2025-01-30T11:42:34+00:00
...
class AboutService(CustomBaseModel):
    ...
    version: str | None = Field(None, description="The version number of the service.", examples=["v209"])
...
class SyntheticTableConfiguration(CustomBaseModel):
    ...
    def mutually_exclusive_fields(cls, values):
        seed_fields = [
            field
            for field in [
                values.sample_seed_connector_id,
                values.sample_seed_dict,
                values.sample_seed_data,
            ]
        ...
        for table_name, synthetic_table in synthetic_table_map.items():
            ...
            if config and config.rebalancing:
                ...
                rebalancing_col = next(
                    (col for col in generator_table.columns or [] if col.name == rebalancing_column), None
                )
