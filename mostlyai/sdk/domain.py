...
#   timestamp: 2025-03-14T13:25:20+00:00
...
class ConnectorAccessType(str, Enum):
    ...
    - `READ_DATA`: This connector allows full read access. As such it can also be used as a source for training a generator.
    - `WRITE_DATA`: This connector allows full read and write access. As such it can be also used as a source for training a generator, as well as a destination for delivering a synthetic dataset.
...
class ConnectorConfig(CustomBaseModel):
    ...
    type: ConnectorType
    access_type: ConnectorAccessType | None = Field("READ_PROTECTED", alias="accessType")
...
class ConnectorWriteDataConfig(CustomBaseModel):
    file: bytes | None = Field(None, description="Binary Parquet file containing the data to write.")
    location: str | None = Field(
        None,
